...
//
// modified to play nicely in CommonJS and out of the whole cloudkid framework
    ...
    p.init = function()
    {
        ...
        if(this.extraData && this.extraData.path)
        {
            ...
            else
            {
                ...
                {
                    console.log("PathParticle: error in parsing path expression");
        ...
        {
            console.log("PathParticle requires a path string in extraData!");
            this.path = null;
        }
    ...
    p.init = function(particleImages, config)
    {
        ...
        if(true && this.particleImages.length > 1)
        {
            for(var i = this.particleImages.length - 1; i > 0; --i)
            {
                ...
                {
                    if(window.console) {
                        console.log("PixiParticles: using particle textures from different images may hinder performance in WebGL");
                    }
